...
import gdal # Self-added
import pandas as pd # Self-added
...
def prep_display(dets_out, img, h, w, path, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            x = (x1+x2)/2 # Added
            y = (y1+y2)/2 # Added
            posX = px_w * x + rot1 * y + xoffset # Added
            posY = rot2 * x + px_h * y + yoffset # Added
            posX_list.append(posX) # Added
            posY_list.append(posY) # Added     
            X_len = abs(px_w*(x2-x1)+rot1*(y2-y1)) # Added
            Y_len = abs(px_h*(y2-y1)+rot2*(x2-x1)) # Added
            X_len_list.append(X_len) # Added
            Y_len_list.append(Y_len) # Added
            areaBox = X_len * Y_len # Added
            areaBox_list.append(areaBox) # Added
            ...
            if args.display_text:
                ...
                class_list.append(_class) # Added
    ...
    ########## For Mask area cal From here modified by Tanaka 
            # mask :1 -> 255 -> then copy to cpu -> then convert from tensor into numpy
            mask_np = (masks[j]* 255).byte().cpu().numpy()
            #print(mask_np)
            ...
            # Refer to https://teratail.com/questions/175770
            # Sum up all cnt area
            area = 0
            for cnt in contours:
    ...
    max_areas = np.array(max_areas)
    arclens = np.array(arclens)
    cxs = px_w*cxs+rot1*cys+xoffset
    cys = rot2*cxs+px_h*cys+yoffset
    areas = areas*px_w**2
    max_areas = max_areas*px_w**2
    arclens = arclens*px_w
    
    for k in range(len(polygons)):
        vector = np.vectorize(np.float)
        polygons[k] = vector(polygons[k])
        polygons[k][:,0] = px_w*polygons[k][:,0]+rot1*polygons[k][:,1]+xoffset
        polygons[k][:,1] = rot2*polygons[k][:,0]+px_h*polygons[k][:,1]+yoffset
        polygons[k] = str(polygons[k])
        polygons[k] = polygons[k].replace('\n','').replace('[[','POLYGON ((').replace('] [',',').replace(']]','))')
        
    
    output_mat = np.array([posX_list, posY_list, X_len_list, Y_len_list, areaBox_list, class_list,cxs, cys, areas, max_areas, arclens],dtype=object) 
    rev_output_mat = output_mat.T
    rev_output_mat = pd.DataFrame(rev_output_mat)
    polygons = pd.Series(polygons)
    rev_output_mat = pd.concat([rev_output_mat, polygons], axis=1)
    
    
    """Until here revised by Tanaka"""
    return img_numpy, rev_output_mat
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy , rev_output_mat = prep_display(preds, frame, None, None, path, undo_transform=False) # revised by Tanaka
    
    """ If you want to make detected pictures, please delte this comment out
